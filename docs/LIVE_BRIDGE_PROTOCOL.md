...
### `edit.plan.validate` / `edit.plan.apply` (0.9.4+, Effect stacks in 0.9.5)

These additive methods accept one to 128 mixed, fully resolved commands with a
single positive `expected_revision`. The wire protocol never performs automatic
cursor/layer placement; the Python `LiveProject` facade sends explicit
zero-based `layer`, `frame`, and positive `length` values.

Supported operations:

- `object.create_from_alias`
- `object.create_from_media_file`, including initial `items`
- `object.update`, including `name` and/or `items`
- `object.move`
- `object.delete`
- `object.effect.add`, including initial items
- `object.effect.set_enabled`

In 0.9.5, each Alias/media creation command may include an ordered
`effects` array (maximum 32 Effects, 128 items per Effect):
  ...
  "effects": [
    {
      ...
      "enabled": true,
      "items": [
        {"effect": "グロー", "item": "強さ", "value": "50.00"}
      ]
    }
  ]
}
...
`scope` is `primary`, `video`, or `audio`; it is checked against the live
catalog and routes video/audio media-group Effects. A native loader may return
separate video/audio objects or one combined `映像再生` object; in the combined
case both scopes intentionally resolve to the same object. Order, duplicates
and the enabled state are significant. The wire endpoint does not translate semantic
parameters: Python resolves the versioned profile to the complete explicit
item list before sending it. Native clients may omit `profile`, but must still
provide exact names and values.

Each command has a unique short `key` used only for result correlation. A newly
created key cannot be the target of a later command in the same plan; all initial
state must be carried by its create command.

Validation occurs inside one SDK read section and checks every existing target,
revision, API/layer/effect lock, media path/readability, item schema available to
the SDK, and the complete final timeline layout. During apply, existing objects
that move or are deleted are temporarily held in verified scratch ranges. This
allows an object created by the plan to occupy a range freed by that same plan
without depending on command ordering or temporary collisions.

For create-time Effects, validation additionally checks unique catalog schema,
domain and limits. Apply reads back the created Effect name, every item value,
enabled state and relative stack order. Missing/ignored/clamped/replaced values
return `EFFECT_READBACK_MISMATCH`; the edit is rolled back and is not reported
as success.

Apply runs inside one `call_edit_section_param()` callback. Success reports
`undo_grouped: true`, `undo_unit: "single_edit_section"`, `atomic: false`, the
fresh revision, per-command status, and a rollback receipt. `atomic` remains
false because the SDK does not guarantee that every linked object created by a
third-party media loader can be identified and deleted independently.

Failure details include `failed_command_index`, current revision when available,
and:
...
The bridge never reports a partial plan as success. Clients must stop and ask
the user for GUI Undo when `gui_undo_required` is true.

### Editing and review methods
...
Accepts one to 128 commands in `params.commands`. This legacy batch endpoint
supports only:
...
Stable Alias batch validation/edit error codes include:
...
Structural-edit error codes additionally include:
